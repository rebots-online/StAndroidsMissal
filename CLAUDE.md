...
- One version string (`MAJOR.MINOR.BUILD`) across `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `gen/android/tauri.properties` — canonical source is `version.txt`; `version.json` is the runtime mirror (splash/About/footer read it). `npm run stamp` **bumps MINOR unconditionally** then stamps everything — the bump lives inside the stamper (SOP: `~/Admin-Manual/versioning/`); MAJOR is hand-edited in `version.txt` per milestone. **The stamp runs once, first, per build invocation** via the entrypoints `build:web` / `build:desktop` / `build:android` — never inside `prebuild` (the Tauri CLI reads `tauri.conf.json` at startup, so an in-flight restamp labels the artifact one version behind its binary). App identifier: `mba.robin.standroidsmissal`. `versionCode` = `MAJOR * 100000 + MINOR` (CC7: never user-facing).
- **Artifact hygiene (CC12): stamped-or-cleaned.** Every artifact left on disk is named `v<version>-standroidsmissal-<qualifier>.<ext>`; anything unstampable is not left behind. Gradle emits stamped APK/AAB names via `base.archivesName` in `src-tauri/gen/android/app/build.gradle.kts` (reads `tauri.properties`) — never reference or preserve `app-universal-release.*`/`intermediary-bundle.aab` names; they exist only transiently inside gitignored `build/`/`target/`. `scripts/collect-artifacts.mjs` stages to `dist/` reading `version.json` verbatim (never recomputes a build number) and is the only managed artifact surface. Note `vite build` wipes `dist/` — build web first, collect after. Stale wrong-version bundles are moved on sight to the universal outbox (`~/outbox/standroidsmissal/` — the cross-project trash bin, never auto-emptied, nothing deleted).
