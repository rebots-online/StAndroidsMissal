...
**Similarity UX (MeaningPanel):** each similar-hit gains (a) **clause focus** — `bestClause(text, query)` (`src/core/vector/clause.ts`) splits on `.:;·` boundaries (min 25 chars/clause), embeds clauses via the existing `embedText`, cosine-argmax; the winning clause renders emphasized with the remainder collapsed behind "more" — hits become quotable at semantic-clause granularity; (b) **`SimilarityGlyph`** (`src/ui/SimilarityGlyph.tsx`) — an icon-sized radial SVG: query at center, this hit's dot at radius ∝ (1−score), sibling hits ghosted, so relative relatedness is visible at a glance; the raw score demotes to a tooltip; (c) **imagery grouping** — `IMAGERY_CONCEPTS` (~15 imagery/metaphor/typology concepts with seed phrases: Light & Darkness, Shepherd & Flock, Vine & Vineyard, Water & Baptism, Bread from Heaven, Lamb & Sacrifice, King & Kingdom, Bridegroom & Bride, Desert & Exile, Mountain of God, Temple & Dwelling, Harvest & Vintage, The Way, Rock & Foundation, Fire & Spirit) merged into `concepts.ts` and ingested exactly like the existing taxonomy, so `INSTANCE_OF` grouping spans OT/Psalms/NT through the verse nodes.
...
**Interpretive layer (generalizes the §7.6 Haydock row):** any public-domain source vendored under `VENDORED/<source>/` (clone-at-home; PROVENANCE.md lock **before** assimilation — INC-15; additive, nothing deleted) ingests via `scripts/ingest-commentary.mjs` into the existing graph tables: nodes `kind='commentary'`, key `commentary:<source>/<Book>/<ch>/<verse>`, `meta.source`, English `text_blocks` (PD translations are English; the Latin column stays NULL — decision 5 governs liturgical text, not commentary), embeddings + FTS rows, `COMMENTS_ON` edges to verse nodes. Query surface `CorpusDb.commentaryFor(book, ch, verse?)`. **This wave ships Haydock 1859** (fulfils the §7.6 `VENDORED/haydock/` row) **and Catena Aurea** (Newman tr. 1841–45); the remaining 13 sources of `DOCS/ScripturalReferences-PublicDomain.md` (Augustine on the Psalms, Bellarmine, à Lapide, Guéranger, Bute Breviary, Roman Catechism, Summa, Ignatius, John of the Cross, de Sales, Imitation, Catholic Encyclopedia, DR/Vulgate whole) are the roadmap over this same schema — zero schema change per added source. BibleView renders commentary read-only beneath verses with source attribution; the connections panel surfaces commentary as `vendored` evidence.
...
| `Pericope` / `PERICOPES` / `SCENARIO_CLUSTERS` | type/consts | `src/core/ontology/parallels.ts:1` | P-T | curated Gospel-parallel spine + scenario clusters | `Pericope { id: string; title: string; cluster: string; refs: { Mt?: string; Mc?: string; Lc?: string; Jo?: string } }` |
...
| tests | node:test | `tests/{clause,parallels}.test.ts` | P-T | clause argmax determinism; pericope table integrity (refs parse, books exist) | `npm test` |
