...
20. **Haydock is an automatic interpretive nucleus, never a manual-tag prerequisite (operator, 2026-07-14).** Every user-facing free-text similarity operation that returns corpus/concept hits also obtains a collateral nearest-Haydock set through `CorpusDb.interpretiveNucleiForText`; finding ordinary hits never suppresses the nuclei and finding nuclei never replaces ordinary hits. Existing whole-commentary embeddings provide the coarse shortlist; `bestClause` re-embeds and reranks clauses so the exposed nucleus is an atomic, quotable thematic unit rather than a whole footnote. The nucleus carries its `COMMENTS_ON` verse anchors and their `INSTANCE_OF` concepts, yielding automatic interpretive/theme evidence even when the user never creates a tag. Manual theme tags remain available as curation/override and may adopt a suggested concept, but no meaning, connection, or theme-surfacing path waits for manual classification. The API operates over the active corpus/module query scope so decision 19's free Haydock module preserves this substrate after the commentary split.
...
**Interpretive layer (generalizes the §7.6 Haydock row):** any public-domain source vendored under `VENDORED/<source>/` (clone-at-home; PROVENANCE.md lock **before** assimilation — INC-15; additive, nothing deleted) ingests via `scripts/ingest-commentary.mjs` into the existing graph tables: nodes `kind='commentary'`, key `commentary:<source>/<Book>/<ch>/<verse>`, `meta.source`, English `text_blocks` (PD translations are English; the Latin column stays NULL — decision 5 governs liturgical text, not commentary), embeddings + FTS rows, `COMMENTS_ON` edges to verse nodes. Query surfaces: `CorpusDb.commentaryFor(book, ch, verse?)` for direct attribution and `CorpusDb.interpretiveNucleiForText(text, k?)` for automatic Haydock thematic nuclei. The latter coarse-ranks whole Haydock records by their committed embeddings, applies `bestClause` to each shortlist record, reranks by clause score with key-stable ties, and attaches verse/concept evidence; no re-ingest or manual tag is required. **This wave ships Haydock 1859** (fulfils the §7.6 `VENDORED/haydock/` row) **and Catena Aurea** (Newman tr. 1841–45); the remaining 13 sources of `DOCS/ScripturalReferences-PublicDomain.md` (Augustine on the Psalms, Bellarmine, à Lapide, Guéranger, Bute Breviary, Roman Catechism, Summa, Ignatius, John of the Cross, de Sales, Imitation, Catholic Encyclopedia, DR/Vulgate whole) are the roadmap over this same schema — zero schema change per added source. BibleView renders commentary read-only beneath verses with source attribution; MeaningPanel and ConnectionsPanel surface nuclei automatically as `vendored · haydock` evidence.

**Journal sidecar workspace (UI elaboration of §7.6 B-C/B-D per the prototype + PRD):** reader context menus (ReaderView, BibleView, OfficeView once SectionReader lands) gain **"✎ Add to Journal/Homily notes"** and **"🖍 Highlight both panes"**. Capture opens `JournalSidecar` (`src/ui/JournalSidecar.tsx`, right split pane on the MeaningPanel pattern): source block (bilingual quote via `alignSelection` + anchor nodeKey/verse ref + capture timestamp), embedded `AccompanimentEditor`, `ConnectionsPanel` (corpus vector hits via `groupedSimilarToText` over note+quote, the user's own past accompaniments via runtime `embedText` against `sidecar_embeddings`, and commentary blocks; every card carries a why-bridge line + evidence chips + add-as-source/dismiss — routes, not bare similarity scores), a destinations row mapping to `exposure` + `OccurrenceSelector` (keep as journal | promote to homily seed | attach to theme/series | schedule for a liturgical occasion), and toast feedback. Dual-pane highlight = lightweight accompaniment (quote + quoteAlt) rendered through the existing `mark.ann` pipeline in **both** panes. Priest/laity vocabulary from settings `mode`. Voice dictation and attachments: schema-ready (`attachments` JSON per PRD), out of scope this wave.
...
| `InterpretiveNucleus` / `CorpusDb.interpretiveNucleiForText` | interface/method | `src/core/data/corpusDb.ts` | P-T | automatic nearest-Haydock clause nuclei collateral to every user-facing text-similarity operation; manual tags optional | `InterpretiveNucleus { key: string; title: string; clause: string; score: number; verseAnchors: string[]; concepts: { conceptId: string; label: string }[]; source: 'haydock' }`; `interpretiveNucleiForText(text: string, k?: number): InterpretiveNucleus[]` (default `k=4`; whole-record shortlist → `bestClause` rerank; key-stable ties) |
