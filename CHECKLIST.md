...
- [ ] **BM.5** Lossless automatic Haydock nucleation of similarity results (decisions 20–21; no manual-tag dependency).
  - **Do:** In `src/core/data/types.ts`, add `NucleusAuthorityKind`, `NucleusSourceManifest`, `InterpretiveNucleus`, `NucleatedSimilarityHit`, `NucleatedSimilarityGroup`, and `NucleatedSimilaritySet` exactly as the P-T entity rows specify. In `src/core/data/corpusDb.ts`, add `CorpusDb.interpretiveNucleiForText(text, opts?)` and `CorpusDb.nucleatedSimilarToText(text, opts?)` exactly as those rows specify and re-export the public result types. The active provider in this wave is Haydock (`source='haydock'`, `authorityKind='scriptural-commentary'`): coarse-rank `commentary:haydock/` records, shortlist at least `k*4`, run `bestClause`, order nuclei by descending `queryScore` with key-ascending ties, and attach unique `COMMENTS_ON` verse anchors plus their deduplicated `INSTANCE_OF` concepts. For `nucleatedSimilarToText`, retain all `similarToText(...candidateK...)` candidates; make each hit atomic with `bestClause`; assign it to the highest-affinity nucleus; calculate `contextScore = 0.7*hit.score + 0.3*nucleusAffinity`; emit at most five groups with at most three representatives each; put every non-representative candidate in the descending-score `tail`. No candidate may occur twice or disappear. With no nuclei, use one stable primary concept per candidate for the same lossless group/tail shape. No re-ingest and no generated labels.
