...
- [ ] **BM.5** Automatic Haydock interpretive nuclei (decision 20; no manual-tag dependency).
  - **Do:** In `src/core/data/corpusDb.ts`, add exported `InterpretiveNucleus` and `CorpusDb.interpretiveNucleiForText(text, k=4)` exactly as the P-T entity row specifies: filter `commentary:haydock/` embeddings, coarse-rank whole records, shortlist at least `k*4`, run `bestClause` against the query, rerank by clause score with key-ascending stable ties, and attach `COMMENTS_ON` verse keys plus unique `INSTANCE_OF` concept `{ conceptId, label }` evidence from those verses. No re-ingest and no generated labels. In every current `groupedSimilarToText` UI consumer (`src/ui/MeaningPanel.tsx`, `ConnectionsPanel` in `src/ui/JournalSidecar.tsx`), obtain/render nuclei collaterally rather than replacing ordinary hits. Direct exact-verse commentary cards also use `bestClause` instead of a leading-character excerpt. Extend `AccompanimentEditor` with `themeSuggestions?: { value; label; evidence }[]`; `JournalSidecar` derives unique concept suggestions from its automatic nuclei and passes them through. Suggestions render automatically and may be clicked into the existing free-form tag list, but ignoring them must not suppress nucleus/meaning/connection results and an empty manual tag list remains fully functional.
  - **Verify:** Add `tests/interpretiveNuclei.test.ts` against the committed corpus: results are Haydock-only, every returned clause is a strict non-empty substring of its commentary text, scores are descending with key-stable ties, verse anchors resolve, concept evidence is deduplicated, and an untagged query still yields render-data for nuclei. Run `npm test` and `npx tsc -b`.
  - **Accept:** `npm test` exits `0` with the nucleus assertions present; `npx tsc -b` exits `0`; semantic inspection confirms MeaningPanel and ConnectionsPanel always request nuclei beside ordinary vector hits and no code path gates them on manual tags.
